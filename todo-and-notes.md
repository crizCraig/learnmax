...
Voyager like
- Updating LLM weights or fine-tuning some layers to update entire abstraction hierarchy
- Explicitly state hierarchy of goals that leads to each action (plan motivation)
- Keep textual history of actions and states and goal hierarchy above
- Allow internal simulation of salient states down to sensory ones with actions to see if they lead to goal, this is the plan
