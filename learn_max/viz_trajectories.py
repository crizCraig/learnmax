...
def show(array_img, name):
    ...
    while True:
        ...
        if e.type == pg.KEYDOWN and e.key == pg.K_RIGHT:
            # TODO: Move window forward one frame
            pass
        elif e.type == pg.QUIT:
            pg.quit()
            raise SystemExit()
...
def main():
    ...
    frame_paths = frame_paths[:WINDOW_LENGTH]
    frame_imgs = []
    ...
    show(win_frame_img, "learnmax predicted trajectories per step")
...
def get_np_surface(frame_path, img_scaling):
    ...
    np_surface = surfarray.array3d(surface)
    return np_surface
